...
### 9.7 画布默认缩放：文字偏小（截图复核提出，未修）

**现在有什么**：`views/qt/theme.hpp` 把颜色和对比度变成了契约，节点盒改成两段式之后，**每一处文字都通过
WCAG AA**。截图复核的结论是「三个节点框中的所有文字都清晰可读」。

**缺什么**：**默认视图下节点太小**，文字虽然合规但在正常查看尺寸下偏小、读起来吃力。C5 的三条要求是
「高对比、大字号、色盲安全」——前两条已经可断言，**「大字号」这一条还没有**：调色板管的是颜色与
对比度，字号与缩放属于画布布局。

**为什么现在不修**：它不是调色板问题，改它要动画布的 `fitInView` 与节点尺寸常量，而这两者与
「同一份图的多张皮」这条设计判据有关——把节点画多大是**视图的选择**，四张皮可以各有各的答案。

**重新开启的条件**：一次真实的教学使用中有人读不清节点标题。届时要做的两件事都不是改颜色：
（1）给 `NodeGraphView` 一个「按可读性缩放」的最小缩放下限，或（2）把节点尺寸常量与字号绑成一个
可配置的投影参数。**判据是「用户读不清」，不是「字看起来小」**——后者在没有真实屏幕与真实视力的
条件下无法判断，而前者可以。
