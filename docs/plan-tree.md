...
（RK4 积分器 + 阻尼/其他积分器的拒绝理由 + `MechanicsBinder`）。

**缺什么**：具体物理模型、具体仪器（含误差模型）、拟合与不确定度传播、渲染项、教师层实验描述、示例图库。
...
### 9.5 `runtime/store` 的 `FitResult`：结构冻结，无生产者也无消费者

**现在有什么**：`FitResult`（系数 + 协方差 + 残差 + R²）与 `Measurement`/`Uncertainty` 的三态
（unknown / exact / standard）——这是闭环里「报告」那一半的数据形状，且已冻结。

**缺什么**：产生它的拟合算法（`plugins/analysis`）与显示它的面板。

**为什么现在不做**：见 §9.1 第 2 条。结构先冻结是对的——它是**契约**，而契约必须在有第一个
实现之前定下来，否则第一个实现会顺带定义契约。

**重新开启的条件**：一次实验需要「对一组读数做线性回归并把斜率的不确定度带进报告」。
那时要做的是：写一个 `plugins/analysis` 里的拟合算法（它是内容），并在测量面板上加一栏
（它读的是已经存在的结构）。**契约不需要动**，这是它现在就被冻结的回报。
